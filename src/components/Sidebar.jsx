...
import { BarChart3, Users, DollarSign, MessageSquare, Bot, Layers, Calendar, FileText, Settings, ChevronRight, Zap, Star, Package, UserCheck, Mail, Receipt, Scale } from "lucide-react";
import { C } from "../constants.js";
export default function Sidebar({ page, setPage, stats, waStatus }) {
  ...
  return (
    <aside style={{background:`linear-gradient(180deg,${C.surface} 0%,#030710 100%)`,borderRight:`1px solid ${C.border}`,width:open?258:66}} className="flex-shrink-0 flex flex-col transition-all duration-300 relative z-20">
      <div style={{borderBottom:`1px solid ${C.border}`}} className="h-16 flex items-center px-4 gap-3">
        ...
        {open&&<div><div style={{color:C.text}} className="font-extrabold text-[15px] tracking-tight leading-none">NexusPro</div><div style={{color:C.indigo}} className="text-[10px] font-semibold mt-0.5 flex items-center gap-1"><Star size={9}/>Premium · IA</div></div>}
      </div>
      <nav className="flex-1 py-3 px-2 overflow-y-auto space-y-0.5">
        {NAV.map((item,i)=>{
          ...
          const active=page===item.id; const color=bc[item.bc]||C.indigo;
          return (
            <button key={item.id} onClick={()=>setPage(item.id)} style={{background:active?C.indigoDim:"transparent",color:active?C.indigo:C.muted,borderLeft:`2px solid ${active?C.indigo:"transparent"}`}} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-r-xl text-[13px] font-medium transition-all hover:text-white relative">

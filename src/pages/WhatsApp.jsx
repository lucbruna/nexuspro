            ...
            <div className="space-y-3">
              {[[queue.length,"Na fila",C.amber],[mensagens.filter(m=>m.tipo==="enviada").length,"Enviadas",C.green],[mensagens.filter(m=>m.tipo==="recebida").length,"Recebidas",C.cyan],[campanhas.length,"Campanhas",C.purple]].map(([v,l,c])=>(
                <div key={l} style={{background:C.card2,border:`1px solid ${C.border}`}} className="rounded-xl p-3 flex items-center justify-between">
                  ...
                  <span style={{color:c,fontFamily:"monospace"}} className="font-extrabold text-lg">{v}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {tab==="enviar"&&(
        <div className="grid grid-cols-2 gap-5">
          <div style={{background:C.card,border:`1px solid ${C.border}`}} className="rounded-2xl p-5 space-y-4">
            ...
            {[["Telefone",sendForm.telefone,v=>setSendForm({...sendForm,telefone:v}),"11999999999"],].map(([l,v,set,ph])=>(
              <div key={l}>
                <label style={{color:C.sub}} className="text-xs font-bold mb-1 block uppercase tracking-wider">{l}</label>
                <input value={v} onChange={e=>set(e.target.value)} placeholder={ph} style={{background:C.surface,border:`1px solid ${C.border}`,color:C.text,width:"100%",borderRadius:12,padding:"10px 14px",fontSize:13,outline:"none"}}/>
              </div>
            ))}
            <div>

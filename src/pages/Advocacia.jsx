...
function applyTemplate(template, processo = {}) {
...
function Campo({ label, value, onChange, type = "text", children }) {
  return (
    <div>
...
export default function Advocacia({ toast, api }) {
  ...
  const [editPeticao, setEditPeticao] = useState(null);
  ...
  return (
    <div className="space-y-4">
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}` }} className="rounded-2xl p-1 flex gap-1">
        {[["processos", "Processos", Briefcase], ["peticoes", "Peticoes", FileText], ["ia", "IA Juridica", Bot], ["arquivos", "Arquivos", FolderOpen], ["contextos", "Contextos", Scale]].map(([id, label, Icon]) => (
          <button key={id} onClick={() => setTab(id)} style={{ background: tab === id ? C.indigoDim : "transparent", color: tab === id ? C.indigo : C.muted, border: `1px solid ${tab === id ? C.indigoBorder : "transparent"}` }} className="flex-1 py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-2">
            <Icon size={13} />{label}
          </button>
        ))}
      </div>

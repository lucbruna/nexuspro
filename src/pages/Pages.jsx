...
export function Agenda({ toast, api }) {
  ...
  const [novoComp, setNovoComp] = useState(null);
  ...
  if(modo==="compromissos") {
    ...
    const porDia=lista.reduce((acc,c)=>{ if(c.data) acc[c.data]=(acc[c.data]||0)+1; return acc; },{});
    return (
...
export function Relatorios({ toast, api }) {
  ...
  const [secoes, setSecoes] = useState(() => Object.keys(SECOES_RELATORIO).reduce((a,k)=>({...a,[k]:true}),{}));
  ...
  const gerarPDF=async()=>{
    ...
    setGerando(true);
    const doc=new jsPDF();
    const ind=[99,102,241],red=[244,63,94],green=[34,197,94],dark=[2,4,8];
    doc.setFillColor(...dark); doc.rect(0,0,210,42,"F");
    doc.setTextColor(...ind); doc.setFontSize(20); doc.setFont("helvetica","bold");
    ...
    if(secoes.resumo){
      doc.setTextColor(...ind); doc.setFontSize(12); doc.setFont("helvetica","bold");
    ...
    if(secoes.financeiro&&dados.transacoes?.length>0){
      ...
      autoTable(doc,{startY:y,head:[["Tipo","Cat.","Descrição","Valor","Data"]],body:dados.transacoes.map(t=>[t.tipo.toUpperCase(),t.categoria,t.descricao,(t.tipo==="entrada"?"+":"-")+fmt(t.valor),t.data]),theme:"striped",headStyles:{fillColor:dark,textColor:ind,fontStyle:"bold"},bodyStyles:{fontSize:9},didParseCell:d=>{if(d.column.index===3&&d.cell.raw?.startsWith("+"))d.cell.styles.textColor=green;if(d.column.index===3&&d.cell.raw?.startsWith("-"))d.cell.styles.textColor=red;}});
      y=doc.lastAutoTable.finalY+12;
    }
    if(secoes.clientes&&dados.clientes?.length>0){
      if(y>220){doc.addPage();y=20;}
      doc.setTextColor(...cyan); doc.setFontSize(12); doc.setFont("helvetica","bold");
    ...
    for(let p=1;p<=pgs;p++){doc.setPage(p);doc.setFillColor(...dark);doc.rect(0,282,210,15,"F");doc.setTextColor(100,100,100);doc.setFontSize(7);doc.text(`${dados.empresa?.nomeEmpresa||"NexusPro"} — Relatório ${PASTAS.find(p=>p.id===pasta)?.label} — Pág ${p}/${pgs}`,14,290);}
      ...
      <div style={{background:C.card2,border:`1px solid ${C.border}`}} className="rounded-2xl p-5 space-y-4">
        ...
        {status === "baixado" && (
          <div className="space-y-3">
            ...
            <button onClick={instalar} style={{background:`linear-gradient(135deg,${C.green},${C.emerald||C.green})`,color:"white"}} className="w-full py-3 rounded-xl text-sm font-extrabold flex items-center justify-center gap-2">

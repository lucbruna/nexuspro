...
export default function Dashboard({ stats, reload, setPage, toast, api }) {
    ...
    <div className="space-y-5">
      {stats.atrasados>0&&(
        <div style={{background:C.redDim,border:"1px solid rgba(244,63,94,0.3)"}} className="flex items-center justify-between rounded-2xl px-5 py-3.5">
          ...
          <div className="flex gap-2">
            <button onClick={()=>setPage("chatia")} style={{background:"rgba(244,63,94,0.12)",color:C.red,border:"1px solid rgba(244,63,94,0.3)"}} className="px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-red-500/20">Chat IA</button>
            <button onClick={()=>setPage("crm")} style={{background:C.red,color:"white"}} className="px-3 py-1.5 rounded-lg text-xs font-bold hover:opacity-90">Ver Clientes</button>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div style={{background:C.card,border:"1px solid rgba(244,63,94,0.15)"}} className="rounded-2xl p-5">
          <div className="flex items-center justify-between mb-3"><div style={{color:C.red}} className="font-bold text-sm flex items-center gap-2"><AlertTriangle size={14}/>Cobranças Urgentes</div><button onClick={()=>setPage("crm")} style={{color:C.red}} className="text-[10px] hover:underline">Ver todos</button></div>
